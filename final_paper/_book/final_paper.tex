...
\caption{\label{fig:unnamed-chunk-4}Linear regression example}
\end{figure}

There are many types of regressions that are extensions of linear
regression. For instance, a multilinear regression factors in more than
one input variable to predict an output variable. Logistic regression is
common in the biological sciences when trying to analyze a dependent
variable that has a binary output (i.e. \(y = 0\) or \(y = 1\)). This
...
\begin{figure}
\centering
\includegraphics{final_paper_files/figure-latex/unnamed-chunk-9-1.pdf}
